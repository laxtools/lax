...
public:
	...
	struct config
	{
		...
		bool enable_loopback = false;		/// 테스트를 위해 send에서 바로 on_recv를 호출
	};
...
public:
	 ...
	 */
	virtual result send(packet::ptr m, const uint8_t* const data, std::size_t len);

private:
	/// cipher, checksum, then send to session
	result send_final(bits_message::ptr mp, const uint8_t* const data, std::size_t len);

	/// session is bound
	virtual void on_bind() override;

	/// session calls this when received data
	virtual result on_recv(const uint8_t* const bytes, std::size_t len) override;
