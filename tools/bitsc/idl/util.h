...
inline int ToUTF8(uint32_t ucc, std::string *out) {
  ...
  for (int i = 0; i < 6; i++) {
    // Max namespace bits this encoding can represent.
    uint32_t max_bits = 6 + i * 5 + static_cast<int>(!i);
    if (ucc < (1u << max_bits)) {  // does it fit?
      // Remaining namespace bits not encoded in the first byte, store 6 namespace bits each
...
inline int FromUTF8(const char **in) {
  int len = 0;
  // Count leading 1 namespace bits.
  ...
  // Grab initial namespace bits of the code.
  int ucc = *(*in)++ & ((1 << (7 - len)) - 1);
  for (int i = 0; i < len - 1; i++) {
    if ((**in & 0xC0) != 0x80) return -1;  // Upper namespace bits must 1 0.
    ucc <<= 6;
    ucc |= *(*in)++ & 0x3F;  // Grab 6 more namespace bits of the code.
  }
