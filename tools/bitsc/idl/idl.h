...
// assurance:
#ifdef __GNUC__
#define CheckedError CheckedError \
...
class Parser : public ParserState {
 public:
  ...
  CheckedError CheckInRange(int64_t val, int64_t min, int64_t max);
...
private:
  ...
  CheckedError Error(const std::string &msg);
  CheckedError ParseHexNum(int nibbles, uint64_t *val);
  CheckedError Next();
  CheckedError SkipByteOrderMark();
  bool Is(int t);
  bool IsIdent(const char *id);
  CheckedError Expect(int t);
  std::string TokenToStringId(int t);
  EnumDef *LookupEnum(const std::string &id);
  CheckedError ParseNamespacing(std::string *id,
                                             std::string *last);
  CheckedError ParseTypeIdent(Type &type);
  CheckedError ParseType(Type &type);
  CheckedError AddField(StructDef &struct_def,
                                     const std::string &name, const Type &type,
                                     FieldDef **dest);
  CheckedError ParseField(StructDef &struct_def);
  CheckedError ParseString(Value &val);
  CheckedError ParseComma();
  CheckedError ParseAnyValue(Value &val, FieldDef *field,
  ...
  #endif  // defined(FLATBUFFERS_CPP98_STL)
  CheckedError ParseTableDelimiters(size_t &fieldn,
                                                 const StructDef *struct_def,
                                                 ParseTableDelimitersBody body,
                                                 void *state);
  CheckedError ParseTable(const StructDef &struct_def,
  ...
  #endif  // defined(FLATBUFFERS_CPP98_STL)
  CheckedError ParseVectorDelimiters(
      size_t &count, ParseVectorDelimitersBody body, void *state);
  CheckedError ParseVector(const Type &type, uoffset_t *ovalue);
  CheckedError ParseNestedFlatbuffer(Value &val, FieldDef *field,
                                                  size_t fieldn,
                                                  const StructDef *parent_struct_def);
  CheckedError ParseMetaData(SymbolTable<Value> *attributes);
  CheckedError TryTypedValue(int dtoken, bool check, Value &e,
                                          BaseType req, bool *destmatch);
  CheckedError ParseHash(Value &e, FieldDef* field);
  CheckedError TokenError();
  CheckedError ParseSingleValue(Value &e);
  CheckedError ParseEnumFromString(Type &type, int64_t *result);
  StructDef *LookupCreateStruct(const std::string &name,
                                bool create_if_new = true,
                                bool definition = false);
  CheckedError ParseEnum(bool is_union, EnumDef **dest);
  CheckedError ParseNamespace();
  CheckedError StartStruct(const std::string &name,
                                        StructDef **dest);
  CheckedError ParseDecl();
  CheckedError ParseService();
  CheckedError ParseProtoFields(StructDef *struct_def,
                                             bool isextend, bool inside_oneof);
  CheckedError ParseProtoOption();
  CheckedError ParseProtoKey();
  CheckedError ParseProtoDecl();
  CheckedError ParseProtoCurliesOrIdent();
  CheckedError ParseTypeFromProtoType(Type *type);
  CheckedError SkipAnyJsonValue();
  CheckedError StartParseFile(const char *source,
                                           const char *source_filename);
  CheckedError ParseRoot(const char *_source,
                                    const char **include_paths,
                                    const char *source_filename);
  CheckedError DoParse(const char *_source,
                                           const char **include_paths,
                                           const char *source_filename,
                                           const char *include_filename);
  CheckedError CheckClash(std::vector<FieldDef*> &fields,
