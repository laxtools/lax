...
template<typename T> inline CheckedError atot(const char *s, Parser &parser,
  ...
  const int64_t min = bits::numeric_limits<T>::min();
  const int64_t max = bits::numeric_limits<T>::max();
...
CheckedError Parser::DoParse(const char *source,
  ...
  for (;;) {
    ...
    if (opts.proto_mode &&
      ...
      auto name = bits::PosixPath(attribute_.c_str());
      EXPECT(kTokenStringConstant);
      // Look for the file in include_paths.
      std::string filepath;
      for (auto paths = include_paths; paths && *paths; paths++) {
        filepath = bits::ConCatPathFileName(*paths, name);
...
bits::Offset<bits::Vector<bits::Offset<
  reflection::KeyValue>>>
    Definition::SerializeAttributes(FlatBufferBuilder *builder,
                                    const Parser &parser) const {
  std::vector<bits::Offset<reflection::KeyValue>> attrs;
