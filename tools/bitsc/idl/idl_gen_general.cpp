...
std::string GenDefaultValue(const Value &value, bool enableLangOverrides) {
  ...
  switch (value.type.base_type) {
    ...
    case BASE_TYPE_ULONG:
    {
      ...
      // Converts the ulong into its namespace bits signed equivalent
...
// Returns the function name that is able to read a value of the given type.
std::string GenGetterForLookupByKey(bits::FieldDef *key_field,
...
std::string GenOffsetGetter(bits::FieldDef *key_field,
                            const char *num = nullptr) {
...
std::string GenLookupKeyGetter(bits::FieldDef *key_field) {
...
std::string GenKeyGetter(bits::FieldDef *key_field) {
  std::string key_getter = "";
...
void GenStruct(StructDef &struct_def, std::string *code_ptr) {
  ...
  code += "\n";
  bits::FieldDef *key_field = nullptr;
  if (struct_def.fixed) {
...
std::string BinaryMakeRule(const Parser &parser,
  ...
  std::string filebase = bits::StripPath(
      bits::StripExtension(file_name));
