...
class CppGenerator : public BaseGenerator {
 public:
  ...
  void GenIncludeDependencies() {
    ...
    for (auto it = parser_.included_files_.begin();
      ...
      if (it->second.empty())
        continue;
      auto noext = bits::StripExtension(it->second);
      auto basename = bits::StripPath(noext);
  ...
  bool generate() {
    ...
    if (parser_.root_struct_def_) {
      ...
      code_ += "  return bits::GetRoot<{{CPP_NAME}}>(buf);";
      code_ += "}";
      code_ += "";
      ...
      if (parser_.opts.mutable_buffer) {
        ...
        code_ += "  return bits::GetMutableRoot<{{STRUCT_NAME}}>(buf);";
        code_ += "}";
        code_ += "";
      }
      ...
      if (parser_.file_identifier_.length()) {
        ...
        code_ += "  return bits::BufferHasIdentifier(";
      ...
      code_ += "    bits::FlatBufferBuilder &fbb,";
      code_ += "    bits::Offset<{{CPP_NAME}}> root) {";
      if (parser_.file_identifier_.length())
      ...
      if (parser_.opts.generate_object_based_api) {
        ...
        code_ += "    const bits::resolver_function_t *res = nullptr) {";
 ...
 private:
  ...
  void GenComment(const std::vector<std::string> &dc, const char *prefix = "") {
    std::string text;
    ::bits::GenComment(dc, &text, nullptr, prefix);
    code_ += text + "\\";
  }
  ...
  std::string GenTypeWire(const Type &type, const char *postfix,
    ...
    if (IsScalar(type.base_type)) {
      ...
      return "bits::Offset<" + GenTypePointer(type) + ">" + postfix;
    }
  }
  ...
  std::string GetUnionElement(const EnumVal &ev, bool wrap, bool actual_type,
    ...
    if (ev.union_type.base_type == BASE_TYPE_STRUCT) {
      ...
      return actual_type
          ? (native_type ? "std::string" : "bits::String")
          : Name(ev);
    } else {
      assert(false);
      return Name(ev);
  ...
  std::string UnionVerifySignature(const EnumDef &enum_def) {
    return "bool Verify" + Name(enum_def) +
           "(bits::Verifier &verifier, const void *obj, " +
           Name(enum_def) + " type)";
  }

  std::string UnionVectorVerifySignature(const EnumDef &enum_def) {
    return "bool Verify" + Name(enum_def) + "Vector" +
           "(bits::Verifier &verifier, " +
           "const bits::Vector<bits::Offset<void>> *values, " +
           "const bits::Vector<uint8_t> *types)";
  }
  ...
  std::string UnionUnPackSignature(const EnumDef &enum_def,
    ...
    return (inclass ? "static " : "") +
           ...
           " type, const bits::resolver_function_t *resolver)";
  }

  std::string UnionPackSignature(const EnumDef &enum_def, bool inclass) {
    return "bits::Offset<void> " +
           (inclass ? "" : Name(enum_def) + "Union::") +
           "Pack(bits::FlatBufferBuilder &_fbb, " +
           "const bits::rehasher_function_t *_rehasher" +
  ...
  std::string TableCreateSignature(const StructDef &struct_def,
    ...
    return "bits::Offset<" + Name(struct_def) + "> Create" +
           Name(struct_def)  +
           "(bits::FlatBufferBuilder &_fbb, const " +
           NativeName(Name(struct_def), &struct_def, opts) +
           " *_o, const bits::rehasher_function_t *_rehasher" +
           (predecl ? " = nullptr" : "") + ")";
  }
  ...
  std::string TableUnPackSignature(const StructDef &struct_def,
    ...
    return NativeName(Name(struct_def), &struct_def, opts) + " *" +
           (inclass ? "" : Name(struct_def) + "::") +
           "UnPack(const bits::resolver_function_t *_resolver" +
           (inclass ? " = nullptr" : "") + ") const";
  }
  ...
  std::string TableUnPackToSignature(const StructDef &struct_def,
    ...
    return "void " + (inclass ? "" : Name(struct_def) + "::") +
           ...
           "_o, const bits::resolver_function_t *_resolver" +
           (inclass ? " = nullptr" : "") + ") const";
  }

  void GenMiniReflectPre(const StructDef *struct_def) {
    code_.SetValue("NAME", struct_def->name);
    code_ += "inline bits::TypeTable *{{NAME}}TypeTable();";
    code_ += "";
  }
  ...
  void GenMiniReflect(const StructDef *struct_def,
    ...
    for (auto it = types.begin(); it != types.end(); ++it) {
      ...
      ts += "{ bits::" + std::string(ElementaryTypeNames()[et]) + ", " +
    ...
    code_ += "inline bits::TypeTable *{{NAME}}TypeTable() {";
    if (num_fields) {
      code_ += "  static bits::TypeCode type_codes[] = {";
      code_ += "    {{TYPES}}";
      code_ += "  };";
    }
    if (!type_refs.empty()) {
      code_ += "  static bits::TypeFunction type_refs[] = {";
      code_ += "    {{REFS}}";
      code_ += "  };";
    }
    if (!vs.empty()) {
    ...
    code_ += "  static bits::TypeTable tt = {";
    code_ += std::string("    bits::{{SEQ_TYPE}}, {{NUM_FIELDS}}, ") +
  ...
  void GenUnionPost(const EnumDef &enum_def) {
    ...
    code_ += "  for (bits::uoffset_t i = 0; i < values->size(); ++i) {";
  ...
  void GenNativeTable(const StructDef &struct_def) {
    ...
    code_ += "struct {{NATIVE_NAME}} : public bits::NativeTable {";
  ...
  void GenTable(const StructDef &struct_def) {
    ...
    for (auto it = struct_def.fields.vec.begin();
      ...
      if (nested) {
        ...
        code_ += "    return bits::GetRoot<{{CPP_NAME}}>(data);";
        code_ += "  }";
      }
    ...
    code_ += "  bool Verify(bits::Verifier &verifier) const {";
  ...
  void GenBuilders(const StructDef &struct_def) {
    ...
    code_ += "  bits::FlatBufferBuilder &fbb_;";
    code_ += "  bits::uoffset_t start_;";
    ...
    // Builder constructor
    code_ += "  explicit {{STRUCT_NAME}}Builder(bits::FlatBufferBuilder &_fbb)";
    code_ += "        : fbb_(_fbb) {";
    ...
    // Finish() function.
    code_ += "  bits::Offset<{{STRUCT_NAME}}> Finish() {";
    code_ += "    const auto end = fbb_.EndTable(start_);";
    code_ += "    auto o = bits::Offset<{{STRUCT_NAME}}>(end);";
    ...
    code_ += "inline bits::Offset<{{STRUCT_NAME}}> "
            "Create{{STRUCT_NAME}}(";
    code_ += "    bits::FlatBufferBuilder &_fbb\\";
    for (auto it = struct_def.fields.vec.begin();
         it != struct_def.fields.vec.end(); ++it) {
      const auto &field = **it;
      if (!field.deprecated) {
    ...
    switch (field.value.type.base_type) {
      ...
      default: {
        ...
        if (cpp_type) {
          ...
          code += "static_cast<bits::hash_value_t>(_e));";
          code += " else ";
        ...
        switch (vector_type.base_type) {
          ...
          case BASE_TYPE_STRUCT: {
            if (IsStruct(vector_type)) {
              ...
              code += "_fbb.CreateVector<bits::Offset<";
              code += WrapInNameSpace(*vector_type.struct_def) + ">> ";
              code += "(" + value + ".size(), ";
  ...
  void GenTablePost(const StructDef &struct_def) {
    ...
    if (parser_.opts.generate_object_based_api) {
      ...
      code_ +=
          "  struct _VectorArgs "
          "{ bits::FlatBufferBuilder *__fbb; "
          "const " +
          NativeName(Name(struct_def), &struct_def, parser_.opts) +
          "* __o; "
          "const bits::rehasher_function_t *__rehasher; } _va = { "
  ...
  static void GenPadding(
      ...
      const std::function<void(int namespace bits, std::string *code_ptr, int *id)> &f) {
    if (field.padding) {
      for (int i = 0; i < 4; i++) {
  ...
  static void PaddingDefinition(int namespace bits, std::string *code_ptr, int *id) {
    *code_ptr += "  int" + NumToString(namespace bits) + "_t padding" +
        NumToString((*id)++) + "__;";
  }

  static void PaddingInitializer(int namespace bits, std::string *code_ptr, int *id) {
    (void)namespace bits;
    *code_ptr += ",\n        padding" + NumToString((*id)++) + "__(0)";
  }

  static void PaddingNoop(int namespace bits, std::string *code_ptr, int *id) {
    (void)namespace bits;
  ...
  void GenStruct(const StructDef &struct_def) {
    ...
    for (auto it = struct_def.fields.vec.begin();
      ...
      if (IsScalar(field.value.type.base_type)) {
        ...
        init_list += "(bits::EndianScalar(" + type + "))";
      } else {
        init_list += "(" + arg_name + ")";
      }
      if (field.padding) {
    ...
    // type name() const { return bits::EndianScalar(name_); }
    for (auto it = struct_def.fields.vec.begin();
         it != struct_def.fields.vec.end(); ++it) {
      const auto &field = **it;
      ...
      auto value = is_scalar ? "bits::EndianScalar(" + member + ")"
                             : member;
      ...
      if (parser_.opts.mutable_buffer) {
        ...
        if (is_scalar) {
          ...
          code_ += "    bits::WriteScalar(&{{FIELD_NAME}}_, "
                  "{{FIELD_VALUE}});";
          code_ += "  }";
        } else {
          code_ += "  {{FIELD_TYPE}}mutable_{{FIELD_NAME}}() {";
...
std::string CPPMakeRule(const Parser &parser, const std::string &path,
                        const std::string &file_name) {
  const auto filebase =
      bits::StripPath(bits::StripExtension(file_name));
